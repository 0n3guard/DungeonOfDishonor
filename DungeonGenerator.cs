...
public class DungeonGenerator : MonoBehaviour
{
    ...
    void SpawnPlayer()
    {
        ...
        GameObject player = GameObject.Instantiate(playerPrefab, new Vector3(0, 0, -1f), Quaternion.identity);
        ...
        room.hasPlayer = true;
        GameObject.Find("GameManager").GetComponent<GameManager>().player = player.GetComponent<PlayerMovement>();

        GameObject.Find("CMvcam1").GetComponent<CinemachineVirtualCamera>().Follow = player.transform;

        player.GetComponent<PlayerManager>().Initialize();
    ...
    void SpawnEnemies()
    {
        ...
        for (int i = 0; i < quantity; i++)
        {
            ...
            gameManager.GetComponent<EnemySpawn>().SpawnEnemy(pos, tileScaling);
        }
    }
            ...
            for (int y = (mapHeight - 1); y >= 0; y--)
            {
                for (int x = 0; x < mapWidth; x++)
                {
                    if (MapManager.map[x, y] != null)
                    {
                        ...
                        {
                            case "Wall":
                                newTile.GetComponent<SpriteRenderer>().sprite = walls[0];
                                break;
                            case "Floor":
                                newTile.GetComponent<SpriteRenderer>().sprite = floors[0];
                                break;
                        }
